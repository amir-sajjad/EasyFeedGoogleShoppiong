...
const ReplyTickets = () => {
    ...
    const getSingleTicketDetails = () => {
        const data = { id: ticketId.ticketId }
        axioshttp.post('ticket/details', data).then(response => {
            if (response.data.status == true) {
                setTicketData(response.data.ticket)
                setUserData(response.data.userSettings)
                setIsLoading(false)
            }
        }).catch(err => {
            console.log(err)
    ...
    const createReply = () => {
        ...
        axioshttp.post('create/reply', replyData, {
            headers: {
    ...
    const closeThisTicket = () => {
        const newData = { id: ticketId.ticketId }
        axioshttp.post('close/ticket', newData).then(res => {
            if (res.data.status == true) {
                setToastMessage('Your Ticket has been closed');
                setShowToast(true);
                getSingleTicketDetails()
            }
        }).catch(error => {
    ...
    useEffect(() => {
        ReactGA.send({ hitType: "pageview", page: "/replyTicket", title: "Reply Ticket" });
        getSingleTicketDetails();
    }, [])
    ...
    return (
        <>
            <div className='p-2 sm:p-4 md:p-12 lg:p-12 xl:p-12 2xl:p-12 '>
                {!isLoading && <Button onClick={HelpCenterOnClick} variant='outlined' style={{ marginBottom: '10px' }} ><ArrowBackIcon /></Button>}
                ...
                {!isLoading && <div className='bg-white p-4 rounded-md' style={{ boxShadow: '0 0 0.3125rem rgba(23, 24, 24, .05), 0 0.0625rem 0.125rem rgba(0, 0, 0, .15)' }}>
                    {ticketData && ticketData.subject ? <div>
                        <p className='font-medium text-base'>[{ticketData && ticketData.ticket_id ? '#' + ticketData.ticket_id : ''}] {ticketData && ticketData.subject ? ' - ' + ticketData.subject : ''}</p>
                        <div className='pt-2'>
                            <div className='flex flex-wrap md:flex-nowrap lg:flex-nowrap xl:flex-nowrap 2xl:flex-nowrap sm:flex-wrap justify-between items-baseline'>
                                <div className='w-[100%] sm:w-[100%] md:w-[75%] lg:w-[75%] xl:w-[75%] 2xl:w-[75%] p-4 rounded-md' style={{ boxShadow: '0 0 0.3125rem rgba(23, 24, 24, .05), 0 0.0625rem 0.125rem rgba(0, 0, 0, .15)' }}>
                                    <div className='flex items-center flex-wrap md:flex-nowrap lg:flex-nowrap xl:flex-nowrap 2xl:flex-nowrap sm:flex-wrap justify-between'>
                                        <div className='flex flex-wrap md:flex-nowrap lg:flex-nowrap xl:flex-nowrap 2xl:flex-nowrap sm:flex-wrap items-center'>
                                            <AccountCircleIcon style={{ color: '#008060', fontSize: '40px' }} />
                                            <p className='ml-2 font-medium'>{ticketData && ticketData.email ? ticketData.email : "johnDoe@gmail.com"}</p>
                                        </div>
                                        <p className='text-sm'>{ticketData && ticketData.created_at ? ticketData.created_at : ''}</p>
                                    </div>
                                    <p className='p-2 text-sm'>{ticketData && ticketData.description ? ticketData.description : ''}</p>
                                    <div className='p-4 overflow-x-scroll flex justify-between'>
                                        {ticketData && ticketData.attachments ? ticketData.attachments.map((value, index) => (
                                            <div className='w-full mr-2'>
                                                <img className='w-[100%] cursor-pointer' onClick={expandImage} src={'/storage/images/' + value.attachment} />
                                            </div>
                                        )) : null}
                                        {/* <div className='w-full mr-2'>
                                            <img className='w-[100%]' src='https://media.sproutsocial.com/uploads/2017/02/10x-featured-social-media-image-size.png' />
                                        </div>
                                        <div className='w-full mr-2'>
                                            <img className='w-[100%]' src='https://media.sproutsocial.com/uploads/2017/02/10x-featured-social-media-image-size.png' />
                                        </div>
                                ...
                                <div className='w-[100%] sm:w-[100%] md:w-[24%] lg:w-[20%] xl:w-[20%] 2xl:w-[20%] rounded-md p-2' style={{ boxShadow: '0 0 0.3125rem rgba(23, 24, 24, .05), 0 0.0625rem 0.125rem rgba(0, 0, 0, .15)' }}>
                                    <div className='p-2'>
                                        <p className='pt-4 font-medium'>Status</p>
                                        <p className='pt-4'>{ticketData && ticketData.status ? ticketData.status : ''}</p>
                                        {ticketData && ticketData.status && ticketData.status == 'open' ? <Button onClick={closeThisTicket} variant='contained' style={{ color: 'white', background: '#d82c0d', marginTop: '10px' }}>
                        ...
                        {ticketData && ticketData.replies ? ticketData.replies.map((element, index) => (
                            <div className='w-[100%] sm:w-[100%] md:w-[75%] lg:w-[75%] xl:w-[75%] 2xl:w-[75%] mt-4 p-4 rounded-md' style={{ boxShadow: '0 0 0.3125rem rgba(23, 24, 24, .05), 0 0.0625rem 0.125rem rgba(0, 0, 0, .15)' }}>
                                <div className='flex  flex-wrap md:flex-nowrap lg:flex-nowrap xl:flex-nowrap 2xl:flex-nowrap sm:flex-wrap justify-between'>
                                    <div className='flex  flex-wrap md:flex-nowrap lg:flex-nowrap xl:flex-nowrap 2xl:flex-nowrap sm:flex-wrap items-center'>
                        ...
                        {(ticketData && ticketData.status == 'open') && <>
                            ...
                            <div>
                                <div className='w-full mt-4'>
                                    ...
                                    <label
                                        class="flex flex-col justify-center py-8 items-center m-auto w-full h-fit px-4 transition bg-white border-2 border-gray-300 border-dashed rounded-md appearance-none cursor-pointer hover:border-gray-400 focus:outline-none">
                        ...
                        <span className="text-xl">Oops, this ticket doesn't exist.</span>
                    </div>}
                </div>}
            </div>
